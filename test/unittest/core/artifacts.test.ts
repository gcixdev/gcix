...
test("report", () => {
  const artifacts = new Artifacts({
    reports: [{ reportType: "coverage_report", file: "cov.xml" }],
  });
...
test("artifacts and report", () => {
  check(
    new Artifacts({
      ...
      reports: [{ reportType: "codequality", file: "code_quality.xml" }],
    }).render(),
    expect,
  );
});
...
test("exceptions", () => {
  expect(() => {
    ...
    /.* is not allowed. Allowed when statements: always,on_failure,on_success/,
  );
});

test("getter and methods", () => {
  const artifact = new Artifacts({
    paths: ["src/artifacts/*", "file.xml", "test.json"],
    excludes: ["src/artifacts/foo", "src/artifacts/bar", "file.xml"],
  });
  ...
  expect(artifact.excludes).toEqual([
    "src/artifacts/foo",
    "src/artifacts/bar",
    "file.xml",
    "added_exclude1",
...
test("equality", () => {
  ...
  expect(artifactToCompareTo.isEqual(artifactNotEqualsToCompareTo)).not.toBe(
    true,
  );
});
...
test("exception if path starts with /", () => {
  expect(() => {
    new Artifacts({ paths: ["/root/path/not/allowed.txt"] });
  }).toThrowError(/Path .* not relative to .*\./);
});
