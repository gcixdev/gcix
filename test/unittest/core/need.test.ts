...
test("other project ref need", () => {
  check(
    new Need({ job: "testjob", project: "foo/bar", ref: "test" }).render(),
    expect,
  );
});
...
test("job with needs", () => {
  ...
  job.addNeeds([
    testJob,
    new Need({ job: "job1" }),
    new Need({ job: "job2", project: "foo/bar" }),
  ]);
  check(
    pipeline.addChildren({ jobsOrJobCollections: [testJob, job] }).render(),
    expect,
  );
});
...
test("collection with needs", () => {
  pipeline
    .addChildren({ jobsOrJobCollections: [testJob] })
    .addChildren({ jobsOrJobCollections: [collection] });
  collection.addChildren({
    jobsOrJobCollections: [
      new Job({ stage: "firstjob", scripts: ["foo"] }),
...
test("needs will be staged", () => {
  ...
  collection = new JobCollection().addChildren({
    jobsOrJobCollections: [new Job({ stage: "second", scripts: ["foobar"] })],
    stage: "SSS",
  });

  const targetJob = new Job({ stage: "target1", scripts: ["foobar"] }).addNeeds(
    [job1, collection],
  );
  const targetCollection = new JobCollection()
    .addChildren({
      jobsOrJobCollections: [
        new Job({ stage: "target2", scripts: ["foobar"] }),
      ],
      stage: "TTT",
    })
    .addNeeds([job1, collection]);
  ...
  const parentCollection = new JobCollection().addChildren({
    jobsOrJobCollections: [collectionWithoutStage],
    stage: "abc",
  });

  const parentCollection2 = new JobCollection();
  parentCollection2.addChildren({
    jobsOrJobCollections: [targetJob],
    stage: "xyz",
  });
  parentCollection2.addChildren({
    jobsOrJobCollections: [targetCollection],
    stage: "xyz",
  });

  const parentParentCollection = new JobCollection().addChildren({
    jobsOrJobCollections: [parentCollection],
    stage: "123",
  });

  pipeline.addChildren({
    jobsOrJobCollections: [parentParentCollection, parentCollection2],
    stage: "final",
  });
  check(pipeline.render(), expect);
});
...
test("needs collection with stage only", () => {
   ...
   **/
  const job1 = new Job({
    scripts: ['echo "I\'m job1"'],
    name: "job",
    stage: "job1_stage",
  });
  const job2 = new Job({
    scripts: ['echo "I\'m job2"'],
    name: "job",
    stage: "job2_stage",
  });
  job2.addNeeds([job1]);
...
test("need name generation", () => {
  ...
  collection1.addChildren({
    jobsOrJobCollections: [job1, job2.addNeeds([job1])],
    name: "seq1name",
    stage: "seq1stage",
  });
  ...
  collection2.addChildren({
    jobsOrJobCollections: [collection1],
    name: "seq2name",
    stage: "seq2stage",
  });
  pipeline.addChildren({
    jobsOrJobCollections: [collection2],
    name: "pipename",
    stage: "pipestage",
  });
...
test("exceptions", () => {
  expect(() => {
    new Need({ artifacts: true });
  }).toThrowError("At least one of `job` or `pipeline` must be set.");
  expect(() => {
    new Need({ job: "foo", ref: "feature_branch" });
  }).toThrowError("`ref` parameter requires the `project` parameter.");
  expect(() => {
    new Need({ project: "gitlab/gitlab", pipeline: "gcix/gcix" });
  }).toThrowError("Needs accepts either `project` or `pipeline` but not both.");
});

test("equality", () => {
  const needToCompareTo = new Need({
    job: "compare",
    artifacts: false,
    project: "gitlab/gitlab",
    ref: "dev",
  });
  const needEqualsToCompareTo = new Need({
    job: "compare",
    artifacts: false,
    project: "gitlab/gitlab",
    ref: "dev",
  });
  const needNotEqualsToCompareTo = new Need({
    artifacts: true,
    job: "notEqualJob",
  });
