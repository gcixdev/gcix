...
describe('image with', () => {
  ...
  test('entrypoint', () => {
    const imageWithEntrypoint = image.withEntrypoint(['/bin/sh', '-c', 'echo "Hello world!']);
    expect(imageWithEntrypoint.entrypoint).toEqual(['/bin/sh', '-c', 'echo "Hello world!']);
  });
});

test('equality', () => {
  const imageToCompareTo = new Image({ name: 'busybox', entrypoint: ['/bin/bash', '-c', 'true'], tag: 'latest' });
  const imageEqualsToCompareTo = new Image({ name: 'busybox', entrypoint: ['/bin/bash', '-c', 'true'], tag: 'latest' });
  const imageNotEqualsToCompareTo = new Image({ name: 'busybox', entrypoint: ['/bin/bash', '-c', 'true'], tag: 'v1.1.1' });
